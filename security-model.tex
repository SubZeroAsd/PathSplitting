...
We assume the parties are connected via authenticated communication channels with guaranteed 
...
that the message was sent by $P$. The adversary can change the order of messages \MESSAGEE{also the content if we do not use $\FF_\smt$} that were sent in the same round, 
...
\TODOE{Figure out whether we actually need $\FF_{\smt}$ and $\FF_{\anon}$ or not for the sake of privacy (currently we do not use them).}
...
\ldots,(\cid_s^{k_s}, U_s^{k_s},v_s^{k_s},t_s^{k_s})\})$ from a user $U_s$, $\FF$ initializes 
the queue $\QQ$, set $\JJ$ and lists $\DD, \HH, \RR, \VV, \UU$ as empty. If the input set is 
empty or x$\sum_{i=1}^{k_s} v_s^i < v$, then $\FF$ aborts. Otherwise, for all $i \in [1,k_s]$, 
it enqueues $(\cid_s^i, U_s,U_s^i,v_s^i,t_s^i)$ to $\QQ$, adds $U_s^i$ to $\RR[U_s]$ and $U_s$ 
to $\UU$. Next, $\FF$ executes the following interactive protocol:
...
%	then $\FF$ aborts.
	...
	\NEWE{$(\mathsf{get}\mathsf{-channel}, \cid_i)$}. If a channel with identifier $\cid_i$ does 
...
%	sends $(\top, h_i^j)$ to $U_i$ and $\sdv$.
	...
	\cash + v_j, \theta_{U_j}.\varphi)$ \TODOP{How to write the conditions here?}, 
