...
initializes the queue $\QQ$, set $\JJ$ and lists $\DD, \HH, \RR, \VV$ as empty. If the input 
set is empty or $\sum_{i=1}^{k_0} v_0^i < v$, then $\FF$ aborts. Otherwise, for all $i \in 
[1,k_0]$, it enqueues $(U_0,U_0^i,v_0^i,t_0^i)$ to $\QQ$, and adds $U_0^i$ to $\RR[U_0]$. Next, 
$\FF$ executes the following interactive protocol:
...
%	then $\FF$ aborts.
	...
	the tuple $(h_i, c_{\langle U^*,U_i \rangle}, v_i, t_i)$ to $U_i$ and $\sdv$, 
	...
	$(U_i,U_i^j,v_i^j,t_i^j)$ to $\QQ$, and adds $U_i^j$ to $\RR[U_i]$.
...
%	sends $(\top, h_i^j)$ to $U_i$ and $\sdv$.
	...
	\item For all $i \in \{n+1,\ldots,1\}$, $\FF$ checks whether $\RR[U_i] \subseteq \JJ$. If 
	this is the case, then $\FF$ adds $U_i$ to $\JJ$, and sends $(\bot, \HH[U_i])$ to $U_i$ and 
	$\sdv$. Otherwise, $\FF$ updates every $d_i \in \DD[U_i]$ as $(-, -, -, h_i)$ with the 
	corresponding $h_i$ from $\HH[U_i]$, adds $d_i$ to $\LL$, and sends $(\top, h_i)$ to $U_i$ 
	and $\sdv$.
\end{enumerate}
}}
