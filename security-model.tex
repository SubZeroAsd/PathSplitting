...
\begin{itemize}[label=-]
	...
	\item If the input set of $U_s$ is empty or $\sum_{i=1}^{k_s} v_s^i < v$, then $\FF$ aborts.
...
%	then $\FF$ aborts.
	...
	\begin{itemize}[label=-]
		...
		and $\sdv$. \MESSAGEE{Call routing ideal functionality here.}
	...
	\fbox{
		\begin{minipage}[t]{0.50\textwidth}
			\procedure[mode=text]{$\Setup(U_r,v):$}{
				...
				\quad set $\mathcal{T}[\lid_i] = \td_i$									\\
				send $(\vec{\ell}, \vec{\td})$ via $\FF_{\smt}$ to $U_s$ and $\sdv$		\\
				send $(v, \vec{\td})$ via $\FF_{\smt}$ to $U_r$ and $\sdv$
			}
			\\ \\
			\procedure[mode=text]{$\Release(\vec{\ell},\vec{k},\vec{w})$}{
				Upon invocation by $U_i$: 												\\
				...
				\quad else:																\\
				\qquad $k_i \gets \updateStatus(\lid_i, (\vec{\ell},\vec{w}), \Rel)$		\\
			...
			\procedure[mode=text]{$\Extract(\vec{\ell},\vec{\td})$}{
				...
				$\getStatus(\lid_i) \neq \Lock$ or 										\\
				$\getNextLock(\lid_i) \neq \bot$ then abort								\\
				if $\validateTrapdoors((\lid_1,\ldots,\lid_j),(\td_1,\ldots,\td_v)) = \bot$ 
				\textcolor{blue}{(or $j \neq v$ in multi-trapdoor case)} then abort		\\
				$\forall i \in [1,j]$: 													\\
				\quad $k_i \gets \updateStatus(\lid_i, \vec{\td}, \Rel)$					\\
				\quad send $(\lid_i, k_i, \Rel)$ to $\getLeft(\lid_i)$ and $\sdv$
			}
		\end{minipage}
		\begin{minipage}[t]{0.45\textwidth}
			\procedure[mode=text]{$\Lock(\vec{\ell}, U_j, v_j)$}{
				Upon invocation by $U_i$: 													\\
				...
				receive $b$ from $U_j$														\\
				if $b = \bot$ then send $(U_j,\bot)$ to $U_i$ and $\sdv$						\\
				else:																		\\
