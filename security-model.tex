...
adversary $\adv$ \MESSAGEP{Here, the adversary/simulator is called $\adv$ and in Figure 1 is called $\sdv$. Pick one for consistency} \MESSAGEE{The adversary mentioned here is the real adversary. One gives the $\corrupt$ interface to the real adversary in simulation-based proofs. Afterwards, once the adversary picks user identifier to corrupt, then in the ideal world it is the simulator's job to corrupt the party. Please check Figure 3 on page 11 and last paragraph on page 12 of \url{https://eprint.iacr.org/2019/1063.pdf}} as a $\ppt$ machine that has access to an interface $\corrupt(\cdot)$, 
...
store, read and update the \REMOVEE{payment channels and} balance of every user.\MESSAGEP{I am not sure about this. $\FF_{\BB}$ is only about managing the coins at the blockchain, no?} \MESSAGEE{You are right, I removed the payment channel part".} It is defined in the 
...
\MESSAGEP{Before, it is mentioned that time was modeled as global clock and now as $|\BB|$?}
\MESSAGEE{I though this is the first point where I define time. May I ask you about which time you are talking? But, I agree that like our above discussion, we need to decide whether we go with time from synchronous communication functionality as you pointed out, or we stick to using time from a blockchain functionality?}


\MESSAGEP{We might want to add the API for each of these ideal functionalities and use it in Figure 1.} \MESSAGEE{Agreed. I will leave here a TODO to explicitly make calls to the hybrid ideal functionalities that we make use of once we decide on the final version of the ideal functionality.}
...
channels. \MESSAGEP{We might want to do an intuitive argument of how to extend it or avoid this point.} $\FF_{\mathsf{MPPCN}}$ makes use of $\FF_{\BB}$, $\FF_{\anon}$ and $\FF_{\syn}$, 
...
\begin{itemize}
	...
	\MESSAGEP{Here is not clear the difference between $\DD$ and $\HH$. Both are storing all the transaction/payments done by each user U??} \MESSAGEE{I tried to clarify it now. The relation is that list $\LL$ holds the whole transaction data of completed transactions, and is defined globally, whereas list $\DD$ stores all transaction data excluding the identifier of the transaction (so in a sense stores "incomplete" transactions which should not be added to list $\LL$ yet), and list $\HH$ merely stores the identifiers $h$ of the transactions and nothing else. So, in a sense if we join the identifiers stored in $\HH$ with the incomplete transaction data from $\DD$, then we will have a complete transaction, which we can add to the list $\LL$.}
...
\MESSAGEP{This needs to be extended when IF clarified} \MESSAGEE{I will try to extend this once we settle on the final version of the ideal functionality.}
...
\MESSAGEP{We might want to omit open channel and close channel. Instead rely on one UC for payment channels and use their open-channel, close-channel and update functions throughout this ideal functionality.}
\MESSAGEE{Sure, we can do that also. I included them here for the sake of completeness. Then, our ideal functionality will only include a $\mathsf{pay}$ interface, and in the security proof we won't need to simulate the open/close operations. But, then I don't think that we need to call the open/close operations from your CCS paper anywhere, right? Because in payment operation we are not opening or closing any channel, we are just using the channels that are already opened.}
...
\begin{enumerate}
	\item For all $i \in \{0,\ldots,n\}$, $\FF$ sets $v_i = \VV[U_i]$ \MESSAGEP{Isn't $\VV[U_i] = 0$ at the beginning?}\MESSAGEE{We start with $U_0$ initially, and in the line above I am setting $\VV[U_0] = v$, so it is not 0 for $U_0$, and for the other users, it gets increases throughout the execution of the loop.}, sends $(\mathsf{split-}
	\mathsf{req}, \sid, \NEWE{U_{n+1}}, v_i)$ to $U_i$ and $\sdv$ \MESSAGEP{$\sdv$ would need to know $U_i$ also, right?}\MESSAGEE{I believe $\sdv$ knows that, right? I mean I have not seen papers sending the identifier of the user to the simulator. They just forward the message that the user will receive also to the simulator. So, I believe you can assume that $\FF$ internally also informs $\sdv$ to which user the message was sent. I don't think we need to be explicit there. For example, this is what is done here \url{https://eprint.iacr.org/2019/1063.pdf} (page 11, Figure 3).}, where $U_i$ replies with $(\mathsf{split}, 
	...
	v^*, t^*, f^*)$.  If such an entry exists, \MESSAGEP{Don't you need to check that the channel has enough balance?}\MESSAGEE{I do check it, but towards the end of the step 1. Here I just check if a channel between the two users exists. I followed this approach since this is what you also had done in your CCS paper. But, sure we can also move balance check here before contacting the user.} then $\FF$ sends the tuple $(h_i^j, c_{\langle 
	U_i,U_i^j \rangle}, v_i^j, t_i^j)$ to the user $U_i^j$ \REMOVEE{via an anonymous communication 
	channel} \MESSAGEP{Why the ACN here?}\MESSAGEE{I think we may not need ACN here, as essentially every user is locking with its next neighbor, so we do not have here sender giving whole path. In your constructions sender prepares some data for every intermediary and receiver, so in actual protocol realization you need to do this via anonymous communication channel to have any anonymity. But I guess in our case we can get rid of it here.} and notifies $\sdv$, where $U_i^j$ replies either with $\top$ or $\bot$. In the 
	...
	lowest $v^*$ \MESSAGEP{Why lowest $v^*$??}\MESSAGEE{Because list $\LL$ stores all the transactions that have successfully completed, hence, the one with the lowest value is the latest state of the channel. Note that we consider only unidirectional channels here}. Lastly, if any of the conditions above is not met or $\VV[U_{n+1}] \neq v$, 
	then $\FF$ aborts.
	
	\item For all $i \in \{n+1,\ldots,1\}$, $\FF$ sends $U_i$ with $\HH[U_i]$ \REMOVEE{through an 
	anonymous communication channel} \MESSAGEP{why ACN here?}\MESSAGEE{Same argument as above.}, and notifies $\sdv$. Each user can reply either with 
...
\MESSAGEE{I thought we are again going for two ideal functionalities as in our A2L, no?}

\caption{Ideal functionality $\FF_{\mathsf{MPPCN}}$ in the $(\FF_{\BB}, \FF_{\anon}, 
\FF_{\syn})$-hybrid model}
\MESSAGEE{The ideal functionality does not consider the fees in value calculations (yet).}
...
neighbors has pulled. \NEWE{We assume that the first part of this property is enforced by the 
environment by making sure that the when a user splits, it cannot split to a value more than it 
is supposed to receive} \REMOVEE{The first part of this property is achieved by our ideal functionality 
in step 1 of $\mathsf{pay}$ interface, by making sure that the when a user splits, it cannot 
split to a value more than it is supposed to receive (text in red)}. \MESSAGEP{We can abstract this part by forcing it in the environment if clearer}\MESSAGEE{I removed the red text from the ideal functionality, but don't know exactly how to formulate that this is enforced by the environment. I wrote the new text above, but sounds vague and unclear.} The second part is achieved 
...
\MESSAGEE{Can we even realize "an intermediary receives at least as many coins as it forwards"? I guess this just means that we consider rationale users who are not going go lock for more coins that they are supposed to receive, right?}
...
\MESSAGEE{I modified it now, but I don't see a proper definition for value privacy in your paper apart from a sentence in the second paragraph of Section III.A.}
...
\MESSAGEE{The definition I gave here is a modification of your CCS'17 definition. However, I got rid of the mathematical symbols that you have in your CCS'17 definition, to keep things more high level, and maybe it made it a bit vague. As for your credit network paper you have linked above, I do not see any definition of relationship anonymity there.}
This is achieved by 
