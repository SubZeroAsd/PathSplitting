...
\begin{itemize}[label=-]
	...
	\item If the input set is empty or $\sum_{i=1}^{k_s} v_s^i < v$, then $\FF$ aborts.
	...
	\fbox{
		\begin{minipage}[t]{0.50\textwidth}
			\procedure[mode=text]{$\Setup(U_r,v):$}{
				...
				\quad insert $(\lid_i, \td_i, U_s, \bot, \Init, \bot)$ to $\LL$			\\
				send $\vec{\ell}$ to $U_s$ and $\sdv$									\\
				send $(v, \vec{\td})$ via $\FF_{\smt}$ to $U_r$ and $\sdv$
			}
			\\ \\
			\procedure[mode=text]{$\Release(\vec{k},\vec{w})$}{
				Upon invocation by $U_i$: 												\\
				Parse $\vec{k}$ as $(k_1,\ldots,k_j)$									\\
				$\forall i \in [1,j]$: 													\\
				\quad if $U_i = U_r$ or $\getRight(\lid_i) \neq U_i$ or 
				$\getStatus(\lid_i) \neq \Lock$ or $\getStatus(\getNextLock(\lid_i)) 
				\neq \Rel$	then send $(\lid_i, \bot)$ to $U_i$ and $\sdv$				\\
				\quad else:																\\
				\qquad $k_i \gets \updateStatus(\lid_i, \vec{w}, \Rel)$					\\
				\qquad send $(\lid_i, k_i, \Rel)$ to $\getLeft(\lid_i)$ and $\sdv$
			}
			\\
			\procedure[mode=text]{$\Extract(\vec{\ell},\vec{\td})$}{
				Upon invocation by $U_r$:												\\
				Parse $\vec{\ell}$ as $(\lid_1,\ldots,\lid_j)$ and $\vec{\td}$ as $(\td_1,\ldots,\td_v)$																\\
				$\forall i \in [1,j]$: if $\getRight(\lid_i) \neq U_r$ or 
				$\getStatus(\lid_i) \neq \Lock$ or 										\\
				$\getNextLock(\lid_i) \neq \bot$ then abort								\\
				if \textcolor{blue}{$j \neq v$ or} $\validateTrapdoors
				((\lid_1,\ldots,\lid_j),(\td_1,\ldots,\td_v)) = \bot$ then abort			\\
				$\forall i \in [1,j]$: 													\\
				\quad $k_i \gets \updateStatus(\lid_i, \vec{\td}, \Rel)$					\\
				\quad send$_\smt$ $(\lid_i, k_i, \Rel)$ to $\getLeft(\lid_i)$ and $\sdv$
			}
		\end{minipage}
		\begin{minipage}[t]{0.45\textwidth}
			\procedure[mode=text]{$\Lock(\vec{\ell}, U_k, v_k)$}{
				Upon invocation by $U_i$: 													\\
				...
				\qquad \quad insert $(\lid', \bot, U_i, U_{j_e}, \Init, \bot)$ into $\LL$	\\
				\qquad \quad $\setRightLock(\lid^*, \lid')$									\\
				\qquad send$_\smt$ $(\lid', \Lock)$ to $\getRight(\lid')$ and $\sdv$	\\
				\qquad receive$_\smt$ $(b)$ from $\getRight(\lid')$					\\
				\qquad if $b = \bot$ then send$_\smt$ $(\getRight(\lid'), \bot)$ 
				to $U_i$	 and $\sdv$															\\
				\qquad else:																	\\
				\qquad \quad $\updateStatus(\lid', \Lock)$									\\
