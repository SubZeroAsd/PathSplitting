...
adversary $\adv$ \MESSAGEP{Here, the adversary/simulator is called $\adv$ and in Figure 1 is called $\sdv$. Pick one for consistency} as a $\ppt$ machine that has access to an interface $\corrupt(\cdot)$, 
...
store, read and update the payment channels and balance of every user.\MESSAGEP{I am not sure about this. $\FF_{\BB}$ is only about managing the coins at the blockchain, no?} It is defined in the 
...
\MESSAGEP{We might want to add the API for each of these ideal functionalities and use it in Figure 1.}

\paragraph{Universal Composability}
We now review the notion of secure realization in the UC framework~\cite{canetti}. 
Intuitively, a protocol realizes an ideal functionality if the \REPLACEP{adversary}{environment?} has no way 
...
\MESSAGEP{It would be good to separate between "global data structures" (i.e., shared for all operations of F), and data structures only for one multi-hop payment}

\NEWP{Global data structures}
...
\NEWP{Multi-hop payment data structures} 
\begin{itemize}
	...
	\MESSAGEP{Here is not clear the difference between $\DD$ and $\HH$. Both are storing all the transaction/payments done by each user U??}
...
\MESSAGEP{This needs to be extended when IF clarified}
...
$\FF$ appends the tuple $(c_{\langle U,U' \rangle}, v, t, f)$ to $\BB$ and $(c_{\langle U,U' 
\rangle}, v, t, h)$ to $\LL$, for some random $h$. Finally, $\FF$ sends $h$ to $U, U'$ and 
$\sdv$. \\

\textbf{Close Channel:} On input $(\close, \sid, c_{\langle U,U' \rangle}, h)$ from a user $U$ 
or $U'$, the ideal functionality $\FF$ parses $\BB$ for an entry $(c_{\langle U,U' \rangle}, 
v, t, f)$ and $\LL$ for an entry $(c_{\langle U,U' \rangle}, v', t', h)$ for $h \neq \bot$. 
If $c_{\langle U,U' \rangle} \in \CC$ or $t' > |\BB|$ or $t' > |\BB|$, then $\FF$ aborts. 
Otherwise, $\FF$ adds the entry $(c_{\langle U,U' \rangle}, v', t')$ to $\BB$ and adds 
$c_{\langle U,U' \rangle}$ to $\CC$. Lastly, $\FF$ sends $(c_{\langle U,U' \rangle}, \bot, h)$ 
to $U, U'$ and $\sdv$. \\
...
\begin{enumerate}
	\item For all $i \in \{0,\ldots,n\}$, $\FF$ sets $v_i = \VV[U_i]$ \MESSAGEP{Isn't $\VV[U_i] = 0$ at the beginning? }, sends $(\mathsf{split-}
	\mathsf{req}, \sid, v_i)$ to $U_i$ and $\sdv$ \MESSAGEP{$\sdv$ would need to know $U_i$ also, right?}, where $U_i$ replies with $(\mathsf{split}, 
	\sid, \{(U_i^1,v_i^1,t_i^1),\ldots,(U_i^{k_i},v_i^{k_i},t_i^{k_i})\})$ for $1 \leq k_i 
	\leq n$. If the set is empty \textcolor{red}{or $\sum_{j=1}^{k_i} v_i^j \geq v_i$}, 
	...
	v^*, t^*, f^*)$.  If such an entry exists, \MESSAGEP{Don't you need to check that the channel has enough balance?} then $\FF$ sends the tuple $(h_i^j, c_{\langle 
	U_i,U_i^j \rangle}, v_i^j, t_i^j)$ to the user $U_i^j$ via an anonymous communication 
	channel \MESSAGEP{Why the ACN here?} and notifies $\sdv$, where $U_i^j$ replies either with $\top$ or $\bot$. In the 
	...
	lowest $v^*$ \MESSAGEP{Why lowest $v^*$??}. Lastly, if any of the conditions above is not met or $\VV[U_{n+1}] \neq v$, 
	then $\FF$ aborts.
	
	\item For all $i \in \{n+1,\ldots,1\}$, $\FF$ sends $U_i$ with $\HH[U_i]$ through an 
	anonymous communication channel \MESSAGEP{why ACN here?}, and notifies $\sdv$. Each user can reply either with 
...
split to a value more than it is supposed to receive (text in red). \MESSAGEP{We can abstract this part by forcing it in the environment if clearer} The second part is achieved 
...
\MESSAGEE{Can we even realize "an intermediary receives at least as many coins as it forwards"?}
