...
\begin{figure*}[!h]
	...
	\fbox{
		\begin{minipage}[t]{0.50\textwidth}
			...
			\\ \\
			\procedure[mode=text]{$\Release(\lid_i,k_i,w_i)$}{
				Upon invocation by $U_i$: 												\\
				if $U_i = U_r$ or $\getRight(\lid_i) \neq U_i$ or 
				$\getStatus(\lid_i) \neq \Lock$ or $\getStatus(\getNextLock(\lid_i)) 
				\neq \Rel$ then send $\bot$ to $U_i$ and $\sdv$							\\
				else:																	\\
				\quad $k_i' \gets \updateStatus(\lid_i, (k_i,w_i), \Rel)$				\\
				\quad if $k_i' = \bot$ then:												\\
				\qquad send $(\mathsf{release},U_i,\bot)$ to $\getLeft(\lid_i)$ 
				and $\sdv$																\\
				\qquad send $\bot$ to $U_i$												\\
				\quad else:																\\
				\qquad send $(\mathsf{release},U_i,(\lid_i, k_i'))$ to $\getLeft(\lid_i)$ 
				and $\sdv$																\\
