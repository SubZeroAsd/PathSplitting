...
\fbox{\procedure{}{
	...
	\< \pcind \pcind \text{Query } \FF_{\mathsf{MPMHL}} \text{ on } (\mathsf{lock},U_r,\vec{\ell},U_i^j,v_i^j) \< \\ [-0.15\baselineskip][]
...
\fbox{\procedure{}{
	...
	\< \text{Query } \FF_{\mathsf{MPMHL}} \text{ on } (\mathsf{setup},U_r,v) \< \\ [-0.15\baselineskip][]
	...
	\< \text{Compute } \vec{w}_s \gets \mathsf{lock}_{U_s}((U_r,\vec{\ell},\{(\cid_s^1, U_s^1,v_s^1,t_s^1),\ldots,(\cid_s^{k_s},U_s^{k_s},v_s^{k_s},t_s^{k_s})\})) \< \\ [-0.15\baselineskip][]
...
\fbox{\procedure{}{
	...
	\< \pcind \text{Query } \FF_{\mathsf{MPMHL}} \text{ on } (\mathsf{extract},\vec{\ell}_j,\vec{\td}) \textcolor{blue}{~(\text{or on } (\mathsf{extract},\vec{\ell},\vec{\td}))} \< \\ [-0.15\baselineskip][]
...
\fbox{\procedure{}{
	...
	\< \pcind \text{Query } \MESSAGEE{routing ideal functionality} \text{ on } (\mathsf{get}\mathsf{-split},U_r,v_i) \< \\ [-0.15\baselineskip][]
	...
	\< \pcind \text{Compute } \vec{w}_i \gets \mathsf{lock}_{U_i}((U_r,\vec{\ell}_i,\{(\cid_i^1, U_i^1,v_i^1,t_i^1),\ldots,(\cid_i^{k_i}, U_i^{k_i},v_i^{k_i},t_i^{k_i})\})) \< \\ [-0.15\baselineskip][]
	...
	\< \pcind \text{Query } \FF_{\mathsf{MPMHL}} \text{ on } (\mathsf{release},\lid_{i-1},k_i,w_i) \< \\ [-0.15\baselineskip][]
...
\begin{figure*}[htb]
	...
	\framebox[0.78\textwidth]{
		\begin{minipage}[t]{0.75\textwidth}
			...
			On input the security parameter $1^\lambda$, a user identifier $U_n$, and payment 
			value $v$, the protocol does the following:
			\begin{itemize}[label=-]
				\item Compute the public parameters $(q, e, \GG_1, \GG_2, \GG_T, g_1, g_2, g_T) 
				\gets \BilGen(1^\lambda)$
				\item Sample $(x,y) \sample \ZZ_q^2$
				\item Compute $\ell_0^{(1)} \gets g_1^{xy}$, $\ell_0^{(2)} \gets g_T^y$ and 
				set the lock $\ell_0 := (\ell_0^{(1)},\ell_0^{(2)})$
				\item Send $\vec{\td} := (x)$ to $U_r$ and $\vec{\ell} := (\ell_0)$ to $U_s$
			\end{itemize}
			\underline{$\langle \Lock_{U_i}(U_r, \vec{\ell},U_j,v_j)\}), \Lock_{U_j}(U_i)
			\rangle$}: On input a receiver identifier $U_r$, vector of locks $\vec{\ell}$, 
			payment value $v_j$ and pair of user identifiers $U_i, U_j$, the protocol does the
			following: 
			\begin{itemize}[label=-]
				...
				\item Send $(U_r,\vec{\ell}_j,v_j)$ to $U_j$ and $\vec{w}$ to $U_i$
			\end{itemize}
			...
			\begin{itemize}
				...
				\item Return $k_i'$ if $\Verify(\ell_i, k_i) = 1$, and $\bot$ otherwise
			\end{itemize}
