	...
	\framebox[0.78\textwidth]{
		\begin{minipage}[t]{0.75\textwidth}
			...
			\begin{itemize}[label=-]
				...
				\item $\pcreturn \vec{\td} := (x)$ to $U_r$ and $\vec{\ell} := (\ell_1)$ to $U_s$
			\end{itemize}
	...
	\framebox[0.78\textwidth]{
		\begin{minipage}[t]{0.75\textwidth}
			...
			\begin{itemize}[label=-]
				...
				\item Sample $(\sk_t,\pk_t) \sample \kgen(1^\lambda)$
				\item Secret share $\sk_t$ as $([sk_t]_1,\ldots,[sk_t]_v)$
				\item Compute $c_t \gets \enc(\pk_t, x_1 \concat \cdots \concat x_v)$
				\item Compute $(c_s^{(1)},\ldots,c_s^{(v)}) \gets (\enc(\pk_{U_r},[sk_t]_1),
				\ldots,\enc(\pk_{U_r},[sk_t]_v))$
				\item Compute $(\ell_1,\ldots,\ell_v) \gets ((\ell_1^{(1)},\ell_1^{(2)},
				\ell_1^{(3)}),\ldots,(\ell_v^{(1)},\ell_v^{(2)},\ell_v^{(3)}))$, where for all 
				$i \in [1,v]$, $\ell_i^{(1)} \gets g_1^{x_i y}$, $\ell_i^{(2)} \gets g_T^y$ and 
				$\ell_i^{(2)} := c_s^{(i)}$
				\item $\pcreturn \vec{\td} := (c_t,v)$ to $U_r$ and $\vec{\ell} := 
				(\ell_1,\ldots,\ell_v)$ to $U_s$
			\end{itemize}
			\underline{$\langle \Lock_{U_i}(U_r, \vec{\ell},U_j,v_j)\}), \Lock_{U_j}()
			\rangle$}: On input a receiver identifier $U_r$, vector of locks $\vec{\ell}$, 
			payment value $v_j$ and a user identifiers $U_j$, the protocol does the following: 
